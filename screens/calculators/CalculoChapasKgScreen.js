...
import { LinhaCalculo } from '../../components/LinhaCalculo';
...
const LINHA_PECA_PADRAO = { id: Date.now(), qtd: '1', comprimentoPeca: '', larguraPeca: '' };
...
export default function CalculoChapasKgScreen({ navigation }) {
  const insets = useSafeAreaInsets();
  const [chapaInfo, setChapaInfo] = useState({ peso: '', comprimento: '3000', largura: '1200' });
  const [pecas, setPecas] = useState([{ ...LINHA_PECA_PADRAO, id: 1 }]);
  const compChapaRef = useRef(null);
  const largChapaRef = useRef(null);

  const handleUpdateChapa = (field, value) => setChapaInfo(current => ({ ...current, [field]: value }));
  ...
  const totalKg = useMemo(() => {
    return pecas.reduce((total, peca) => {
      const B6 = parseLocaleFloat(peca.qtd);
      const C6 = parseLocaleFloat(peca.comprimentoPeca);
      const D6 = parseLocaleFloat(peca.larguraPeca);
  ...
  const handleSaveCalculation = async () => {
    ...
    const newEntry = { id: Date.now(), tipo: 'ChapasKg', total: totalKg, chapaInfo, pecas, };
    try {
    ...
    <SafeAreaView style={styles.safeArea}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <View style={[styles.header, { paddingTop: insets.top }]}>
          ...
          <Text style={styles.headerTitle}>Chapas por Peso (Kg)</Text>
          <View style={{width: 24}}/>
        </View>
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Dados da Chapa (Mestre)</Text>
            <InputMestre label="Peso da Chapa" value={chapaInfo.peso} onChangeText={(val) => handleUpdateChapa('peso', val)} unit="Kg" returnKeyType="next" onSubmitEditing={() => compChapaRef.current.focus()} blurOnSubmit={false} />
            <InputMestre ref={compChapaRef} label="Comprimento da Chapa" value={chapaInfo.comprimento} onChangeText={(val) => handleUpdateChapa('comprimento', val)} unit="mm" returnKeyType="next" onSubmitEditing={() => largChapaRef.current.focus()} blurOnSubmit={false} />
            <InputMestre ref={largChapaRef} label="Largura da Chapa" value={chapaInfo.largura} onChangeText={(val) => handleUpdateChapa('largura', val)} unit="mm" returnKeyType="done" onSubmitEditing={() => Keyboard.dismiss()} />
          </View>
          <View style={styles.card}>
            ...
            {pecas.length === 0 ? <EmptyState /> : pecas.map(item => <LinhaCalculo key={item.id} item={item} onUpdate={handleUpdatePeca} onDelete={() => handleDeletePeca(item.id)} chapaInfo={chapaInfo} />)}
          </View>
        </ScrollView>
